...
import { getRandom, randomVector } from "./components/tools";
class Engine {
        ...
        for (let i = 0; i < this.stepsPerUpdate; i++) {
            for (let astre of this.bodies) {
                ...
                for (let target of this.bodies) {
                    if (target != astre)
                        forceSomme.addInPlace(this.computeForce(astre, target));
                }
...
function createEnv() {
    ...
    let simulation = new Engine(10, canvas);
...
window.addEventListener("DOMContentLoaded", () => createEnv());
